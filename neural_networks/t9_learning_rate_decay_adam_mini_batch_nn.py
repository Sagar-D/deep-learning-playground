...
class NeuralNetwork() :
    ...
    def __init__(
        self,
        layer_dimension: tuple,
        learning_rate=0.01,
        min_cost_delta=1e-10,
        max_learning_iterations=10000,
        enable_l2_regularization=True,
        regularisation_rate=1e-3,
        save_model_flag=True,
        save_model_path="",
        enable_validation=True,
        momentum_beta=0.9,
        rmsprop_beta=0.9,
        learning_decay_rate=0.001,
    ):
        ...
        self.learning_decay = learning_decay_rate
    ...
    def __optimize(self, learning_rate, parameters, momentum_derivatives, rmsprop_derivatives) :
        """
        ...
        """
        for l in range(1, self.L+1) :
            parameters["W"+str(l)] -= learning_rate * momentum_derivatives["dW" + str(l)] / np.sqrt(rmsprop_derivatives["dW"+str(l)] + 1e-8)
            parameters["b"+str(l)] -= learning_rate * momentum_derivatives["db" + str(l)] / np.sqrt(rmsprop_derivatives["db"+str(l)] + 1e-8)
    ...
    def train(self, X, Y, batch_size= 256, initail_parameters=None, X_valid=None, Y_valid=None):
        ...
        for self.iteration in range(self.max_learning_iterations) :
            ...
            X_batch = X[:, curr_batch_index: curr_batch_index + batch_size]
            Y_batch = Y[:, curr_batch_index: curr_batch_index + batch_size]
            ...
            for l in range(1, len(self.layer_dims)):
                momentum_derivatives["dW" + str(l)] = (self.momentum_beta * momentum_derivatives["dW"+str(l)] + (1-self.momentum_beta) * derivatives["dW"+str(l)]) / (1-self.momentum_beta**self.iteration+1)
                momentum_derivatives["db" + str(l)] = (self.momentum_beta * momentum_derivatives["db"+str(l)] + (1-self.momentum_beta) * derivatives["db"+str(l)]) / (1-self.momentum_beta**self.iteration+1)
                rmsprop_derivatives['dW'+str(l)] = (self.rmsprop_beta * rmsprop_derivatives['dW'+str(l)] + (1-self.rmsprop_beta) * np.square(derivatives['dW'+str(l)]) ) / (1-self.rmsprop_beta**self.iteration+1)
                rmsprop_derivatives['db'+str(l)] = (self.rmsprop_beta * rmsprop_derivatives['db'+str(l)] + (1-self.rmsprop_beta) * np.square(derivatives['db'+str(l)]) ) / (1-self.rmsprop_beta**self.iteration+1)

            current_learning_rate = self.learning_rate / (1 + (self.iteration * self.learning_decay))
            self.parameters = self.__optimize(current_learning_rate, self.parameters, momentum_derivatives, rmsprop_derivatives)
            prev_cost = cost
            ...
            if self.iteration % 100 == 0 :
                ...
                print(f"Iteration : {self.iteration} | Cost : {cost} | Current learning rate = {current_learning_rate}")
