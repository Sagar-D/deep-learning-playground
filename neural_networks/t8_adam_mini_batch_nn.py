...
class NeuralNetwork() :
    ...
    def __init__(self,layer_dimension: tuple, learning_rate = 0.01, min_cost_delta = 1e-10, 
                 max_learning_iterations = 10000, enable_l2_regularization=True, regularisation_rate=1e-3, save_model_flag = True, save_model_path="", enable_validation=True, momentum_beta = 0.9, rmsprop_beta = 0.9):
    ...
    def __optimize(self, parameters, momentum_derivatives, rmsprop_derivatives) :
        """
        ...
        """
        for l in range(1, self.L+1) :
            parameters["W"+str(l)] -= self.learning_rate * momentum_derivatives["dW" + str(l)] / np.sqrt(rmsprop_derivatives["dW"+str(l)] + 1e-8)
            parameters["b"+str(l)] -= self.learning_rate * momentum_derivatives["db" + str(l)] / np.sqrt(rmsprop_derivatives["db"+str(l)] + 1e-8)
    ...
    def train(self, X, Y, batch_size= 256, initail_parameters=None, X_valid=None, Y_valid=None):
        ...
        for self.iteration in range(self.max_learning_iterations) :
            ...
            self.parameters = self.__optimize(self.parameters, momentum_derivatives, rmsprop_derivatives)
            prev_cost = cost
